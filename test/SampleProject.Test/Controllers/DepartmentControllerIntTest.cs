...
namespace SampleProject.Test.Controllers
{
    public class DepartmentsControllerIntTest
    {
        ...
        public async Task UpdateDepartment()
        {
            ...
            var response = await _client.PutAsync($"/api/departments/{_department.Id}", TestUtil.ToJsonContent(updatedDepartment));
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        public async Task UpdateNonExistingDepartment()
        {
            ...
            var response = await _client.PutAsync("/api/departments/1", TestUtil.ToJsonContent(_department));
            response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
