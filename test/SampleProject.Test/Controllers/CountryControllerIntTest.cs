...
namespace SampleProject.Test.Controllers
{
    public class CountriesControllerIntTest
    {
        ...
        public async Task UpdateCountry()
        {
            ...
            var response = await _client.PutAsync($"/api/countries/{_country.Id}", TestUtil.ToJsonContent(updatedCountry));
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        public async Task UpdateNonExistingCountry()
        {
            ...
            var response = await _client.PutAsync("/api/countries/1", TestUtil.ToJsonContent(_country));
            response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
