...
namespace SampleProject.Test.Controllers
{
    public class JobHistoriesControllerIntTest
    {
        ...
        public async Task UpdateJobHistory()
        {
            ...
            var response = await _client.PutAsync($"/api/job-histories/{_jobHistory.Id}", TestUtil.ToJsonContent(updatedJobHistory));
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        public async Task UpdateNonExistingJobHistory()
        {
            ...
            var response = await _client.PutAsync("/api/job-histories/1", TestUtil.ToJsonContent(_jobHistory));
            response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
