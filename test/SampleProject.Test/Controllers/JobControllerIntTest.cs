...
namespace SampleProject.Test.Controllers
{
    public class JobsControllerIntTest
    {
        ...
        public async Task UpdateJob()
        {
            ...
            var response = await _client.PutAsync($"/api/jobs/{_job.Id}", TestUtil.ToJsonContent(updatedJob));
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        public async Task UpdateNonExistingJob()
        {
            ...
            var response = await _client.PutAsync("/api/jobs/1", TestUtil.ToJsonContent(_job));
            response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
