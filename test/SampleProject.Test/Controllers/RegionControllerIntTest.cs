...
namespace SampleProject.Test.Controllers
{
    public class RegionsControllerIntTest
    {
        ...
        public async Task UpdateRegion()
        {
            ...
            var response = await _client.PutAsync($"/api/regions/{_region.Id}", TestUtil.ToJsonContent(updatedRegion));
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        public async Task UpdateNonExistingRegion()
        {
            ...
            var response = await _client.PutAsync("/api/regions/1", TestUtil.ToJsonContent(_region));
            response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
